...
class DataLoader(object):
    ...
    def format_file_list(self, data_root, split):
        with open(data_root + '/%s.txt' % split, 'r') as f:
            # 获取图像帧
            frames = f.readlines()
        subfolders = [x[:-1] for x in frames]
        image_file_list = []
        cam_file_list = []
        for subfolder in subfolders:
            imgs_path = glob(os.path.join(self.dataset_dir, subfolder, '*.jpg'))
            cams_path = glob(os.path.join(self.dataset_dir, subfolder, '*cam.txt'))
            for img_path in imgs_path:
                image_file_list.append(img_path)
            for cam_path in cams_path:
                cam_file_list.append(cam_path)
        all_list = {}
